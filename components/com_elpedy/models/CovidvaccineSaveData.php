...
class CovidvaccineSaveData extends JModelDatabase {   
...
//        print_r($data);
        $tb = Table::getInstance("covidvaccinetransaction");        
        $tb -> bind ( $data );
        $tb -> store();
    ...
    protected function saveVaccines($state) {
        ...
        if ( $do -> covidVaccineRecRejId > 0 ) {
            $db -> updateObject('#__CovidVaccineRecRej', $do, 'covidVaccineRecRejId');
        }
        else {
            $db ->insertObject('#__CovidVaccineRecRej', $do); 
        }
