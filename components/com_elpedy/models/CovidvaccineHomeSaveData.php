...
class CovidvaccineHomeSaveData extends JModelDatabase {   
...
//        print_r($data);
        $tb = Table::getInstance("covidvaccinehometransaction");        
        $tb -> bind ( $data );
        $tb -> store();
    ...
    protected function saveVaccines($state) {
        ...
        if ( $do -> covidVaccineRecRejId > 0 ) {
            $db -> updateObject('#__CovidVaccineHomeRecRej', $do, 'covidVaccineRecRejId');
        }
        else {
            $db ->insertObject('#__CovidVaccineHomeRecRej', $do); 
        }
