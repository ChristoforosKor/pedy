...
class CovidvaccineHomeListData extends JModelDatabase {   
    ...
    public function getVaccineTransation($refDate, $selectedHUID, /*$selectedSectorId,*/ $db, $query) {
        ...
        $query -> select ( "ct.CovidVaccineTransactionId, c.CovidVaccineId, c.CovidVaccineDesc, ct.ClinicIncidentGroupId, ct.Quantity,ct.CovidVaccineCompanyId")
                ->from( "CovidVaccine c " )
                -> innerJoin ("CovidVaccineHomeTransaction ct on ct.CovidVaccineId = c.CovidVaccineId and ct.RefDate between " . $db -> quote($refDate . " 00:00:00") . " and "  . $db -> quote($refDate . " 23:59:59") 
 ...
 //     exit;
   ...
   public function getManufacturers($id_health_unit, $db) {
       ...
       $query -> select ( "vc.CovidVaccineCompanyId, cvhr.HealthUnitId, vc.CovidVaccineCompanyDesc")
               ...
               -> WHERE ("HealthUnitId = " . $id_health_unit . " and (vc.CovidVaccineCompanyId = 3 or vc.CovidVaccineCompanyId = 1 or vc.CovidVaccineCompanyId = 9 or vc.CovidVaccineCompanyId = 10 or vc.CovidVaccineCompanyId = 12)");
