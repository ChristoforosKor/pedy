        ...
        if (isset($this->newData[$departmentId][$clinic->ClinicId])):
            ...
            foreach ($this->newData[$departmentId][$clinic->ClinicId] as $personelId => $personel):
                ?>
                                ...
                                <?php
                                foreach ($notSummedIncidents as $incident):
...
foreach ($this->reformedDataInGroups as $clinicGroup=>$clinicGroupData): 
    $groupIncidents = $clinicGroupData['head'];
    ?>
     <table class="table table-bordered table-striped clinicTransaction">
                <thead>
                    <tr class="info">
                        <th colspan="<?php echo count($groupIncidents) + 1; ?>"><?php echo $clinicGroup; ?></th>
                    </tr>
                    <tr>
                        <th><?php echo Text::_('COM_ELG_PEDY_CLINIC'); ?></th>
    <?php foreach ($groupIncidents as $incident) : ?>
                        <th><?php echo $incident; ?></th>
    <?php endforeach; unset($incident);?>
                    </tr>
                </thead>
                <tbody>
    <?php foreach ($clinicGroupData['clinics'] as $clinic => $clinicData): ?>
                    <tr>
                        <th><?php echo $clinic; ?></th>
                    
        <?php foreach ($groupIncidents as $incident): ?>
                        <td><?php echo $this->extractValue($clinicGroup, $clinic, $incident); ?></td>
        <?php endforeach;unset($incident);?>
                    </tr>
    <?php endforeach; unset($clinic); unset($clinicData); ?>
                </tbody>
     </table>
            <hr>                   
<?php endforeach;unset($clinicGroup); unset($clinicGroupData) ?>
    ...
    <?php foreach ($this->incidents as $incident): ?>
                ...
                <tbody id="elgRTClinical" >
    <?php foreach ($clinicsSummedNoEx as $clinic): ?>
                        <tr>
                            <th><?php echo $clinic->Clinic; ?></th>
                        <?php
...
//							
    ...
    ?>
                ...
                <tbody id="elgRTClinical" >
    <?php foreach ($clinicsSummedEx as $clinic): ?>
                        <tr>
                            <th><?php echo $clinic->Clinic; ?></th>
                        <?php
...
<div class="clearfix"></div>
        <?php
        include JPATH_COMPONENT_SITE . '/layouts/partmissing.php';
