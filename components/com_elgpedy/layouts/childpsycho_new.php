...
/** checkers * */
// if (isset($this->dataClinical[1][0][0][4])):

 //    $rowValue = $this->dataClinical[1][0][0][4];
...
echo '<section id="elgdata"><div class="row"><div class="col-md-12"><table class="table table-hover table-bordered clinicTransaction" ><tbody><tr><th class="info">', JText::_('COM_ELG_PEDY_CHECKER'), '</th><td id="elginc0-4" class="col-md-2 ', $this -> checker, ' editable" data-inputclass="edit-text" >', $this -> checker, '</td></tr></tbody></table></div><div class="clearfix"></div></div><div class="row">';
...
foreach ($this->fields->clinics['notSummed'] as $clinic):
    ...
    echo getHeader( $this -> reformedGroups, $clinic, $this -> fields -> incidents );
    echo '<tfoot><tr><td colspan="' . (count($this->fields->incidents) + 2) . '"><button type="button" class="addDoctor btn btn-default">Προσθήκη Ιατρού</button></td> </tr>';
    ...
    foreach ($this->fields->doctors as $doctor):
        ...
        if ( isset( $dataClinicalDepartment[ $clinic->ClinicId] [ $doctor['PersonelId'] ] ) || ( $clinic -> ClinicId == $doctor ['ClinicTypeId'] ) ):
            echo pdCSRow($doctor, $this -> fields -> incidents, $clinic -> ClinicId, $dataClinicalDepartment, $this -> refDate, $this -> healthUnitId, $this -> reformedGroups);
        endif;
...
function getHeader ( $groups, $clinic, $incidents ) 
{
    $html = '';
    $cCount = 0;
...
<script type="text/javascript">
    var clickedClinic = 0;
    var incidents = <?php echo json_encode($this->fields->incidents); ?>;
...
foreach($this->fields->clinics['summed'] as $clinic):	
	...
	foreach($this->fields->incidents as $incident):                                
                                ...
                                echo getRowSumed(  $clinic->ClinicId, $incident, $this -> reformedGroups, $dataClinicalDepartmentSumed[$clinic->ClinicId], $groupsIds, ( $max > 0 ? ' colspan="' . $max . '"' : '' ) );
                endforeach;
	unset($incident);
	echo '</tbody></table></div>';
	if($elgcnt % 2 === 0)
