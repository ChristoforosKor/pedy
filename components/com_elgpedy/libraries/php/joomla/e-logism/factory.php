...
class Factory
{
     ...
     * @return JController. The controller requested or the default controller if the requested controller not found.
     */
    
	public static function getController(JInput $input = null, JApplicationBase $app = null, Array $paths = null)
    {
        $controller = null;
        if($paths === null) 
        {
		...
		$appData = self::getAppData($input);
		$controllerName = $appData['controller'];	
		foreach($paths as $path)
        {
			if (file_exists($path . strtolower($controllerName) . '.php'))
            {
				...
				$controller = $appData['componentname'] . 'Controller' . $controllerName;
                break;
            }
        }
       	unset($path);
    ...
    {
        require_once JPATH_COMPONENT . '/libraries/php/joomla/e-logism/controllers/default.php';
    ...
    {
        require_once JPATH_COMPONENT .  '/libraries/php/joomla/e-logism/models/default.php';
     ...
     */
    public static function getView($prefix, $viewName, JModel $model = null, $format = 'html', Array $paths = null)
    {
		if($paths === null)
        {
        ...
        foreach($paths as $path)
        {
            ...
            {
                require_once $path . strtolower($viewName) . '/view.' . $format . '.php';
