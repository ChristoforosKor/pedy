...
class ElgPedyModelPersonelsCommittees extends PedyDataEdit
{
	function getState() 
	{
            ...
            $data->committees = $this->pedyDB->loadObjectList(); 
            $query->clear();
            $query->setQuery('select s.* from (select @p1:= ' . $state->get('HealthUnitId', 0) . ' p) parm ,  pedy.vw_PersonelScheduleByHU s where  s.FromDate >=' . $this->pedyDB->Quote($state->get('start')) . ' and s.ToDate <=' . $this->pedyDB->Quote($state->get('end')) );
