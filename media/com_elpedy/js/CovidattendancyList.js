...
export function init(optionsIn) {
    ...
    CovidAttendancyOptions.btSearch = document.getElementById('btVaccListSearch');
    CovidAttendancyDelContainer = document.querySelector('.covid-attendance-delete-model');
    CovidAttendancyOptions.table.bootstrapTable({
        url:  CovidAttendancyOptions.dataUrl 
        ,pagination: true
        ,sidePagination: 'server' 
        , idField: 'id'
        , toolbar: '#CovidAttendancyListToolbar'
...
//        }          
    ...
    CovidAttendancyOptions.btSearch.addEventListener('click', function() {
...
function formatEdit(val) {
    return '<a href="' + CovidAttendancyOptions.editUrl + '&id=' + val + '" class="btn btn-primary" >Επεξεργασία</a> <button type="button"  class="btn btn-danger del-button"    onclick="delQuestion(this)" value="' + val + '" >Διαγραφή</button>';
}
...
export function delQuestion( trg ) {
   ...
   CovidAttendancyDelContainer.querySelector('.text-info').textContent = row.cells[0].textContent + ' - ' + row.cells[1].textContent + ' - ' + row.cells[2].textContent + ' -  ' + row.cells[3].textContent  + ' -  ' + row.cells[4].textContent;
   jQuery(CovidAttendancyDelContainer).modal('show')
}

export function delRec() {
  
    jQuery.post(CovidAttendancyOptions.delUrl, {id: document.getElementById('delId').value }, function(response){
          ...
          if ( ! elgsJS.hasErrors(response) ) {
            setTimeout( function() { jQuery(CovidAttendancyDelContainer).modal('hide') }, 1500);
            CovidAttendancyOptions.table.bootstrapTable('refresh');
        }          
    });
}
