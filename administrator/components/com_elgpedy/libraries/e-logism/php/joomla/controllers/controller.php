...
class Controller extends JControllerBase{
    ...
    /**
     * Factory method
	 * Creates and executes the proper controller
     */
    public function execute() {
        $input = $this->getInput();
        $appData = self::initAppData(self::getFilteredData($input));
        $input->set('appData', $appData);
        $controller = Factory::getController($appData['controller'], $appData['componentname'], $input, $this->getApplication());
        $controller->execute();
    }
}
